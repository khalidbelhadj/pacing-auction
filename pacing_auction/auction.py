...
class Auction:
    ...
    # Generated
    v: NDArray[np.float64] = field(init=False)  # (n, m)
    b: NDArray[np.float64] = field(init=False)  # (1, n)
    alpha_q: NDArray[np.uint64] = field(init=False)  # (1, n)
    ...
    def __post_init__(self) -> None:
        ...
        if self.no_budget:
            self.b = np.full(self.n, float("inf"))

        self.alpha_q = np.array(
    ...
    def utility(
        ...
        Returns:
            ...
            The utility vector
        """
        return np.sum(np.multiply(self.v - p, x), axis=1)
    ...
    def responses(self, max_cycles: int = 1) -> BRDResult:
        ...
        while True:
            ...
            # Cycle detection
            current_state = tuple(self.alpha_q)
            if current_state in seen:
                cycles_found += 1
                cycle_length = iteration - seen[current_state]
                cycle_start_iterations.append(seen[current_state])
